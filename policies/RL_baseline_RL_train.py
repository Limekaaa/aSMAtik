...
class RLPolicies(nn.Module):
    ...
    """
    def logits_to_action(self, logits, dynamic_mask=None, agent=None):
        # Categorical handles raw logits perfectly for PPO
        penalty = (1.0 - dynamic_mask) * -1e9
        logits = logits + penalty

        if agent is not None and agent.robot_type == "yellow":
            temperature = ws.yellow_rl_temperature
            valid_action_indices = (dynamic_mask > 0.5)
            logits[0, valid_action_indices] = logits[0, valid_action_indices] / temperature
        elif agent is not None and agent.robot_type == "green":
            temperature = ws.green_rl_temperature
            valid_action_indices = (dynamic_mask > 0.5)
            logits[0, valid_action_indices] = logits[0, valid_action_indices] / temperature
        elif agent is not None and agent.robot_type == "red":
            temperature = ws.red_rl_temperature
            valid_action_indices = (dynamic_mask > 0.5)
            logits[0, valid_action_indices] = logits[0, valid_action_indices] / temperature
    ...
    def _knowledge_to_input(self, agent):
        ...
        # Flatten and combine
        device = next(self.parameters()).device
        combined_input = torch.cat([spatial_img.flatten(), inv_vect, message_vect]).cpu()
        ...
        for i, action in enumerate(self.available_actions):
            ...
            elif a_type in ['send_message', 'read_message']:
                if ws.communication_allowed:
                    mask[i] = 1.0
        ...
        return combined_input, mask.cpu()
    ...
    def deliberate(self, agent):
        ...
        chosen_action, action_idx, log_prob = self.logits_to_action(logits, dynamic_mask, agent)
